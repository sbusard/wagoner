...
def extract_tree(table, length, prefix=0, flatten=False):
    """
    Extract from table a tree for word length, taking only prefixes of prefix
    length (if greater than 0) into account to compute successors.
...
def trim_tree(tree):
    """
    Remove the dead branches of tree, that is, the resulting tree accepts the
    same language as the original one (that is, the same words that end with
    the < character), but parts of the tree that lead to nothing are removed.
